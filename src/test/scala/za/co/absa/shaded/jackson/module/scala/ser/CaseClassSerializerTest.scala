...
import za.co.absa.shaded.jackson.annotation.JsonProperty.Access
import za.co.absa.shaded.jackson.annotation._
import za.co.absa.shaded.jackson.databind.{ObjectMapper, PropertyNamingStrategy}
...
class CaseClassSerializerTest extends SerializerTest {
  ...
  it should "serialize a case class with JsonTypeInfo" in {
    serialize(JsonTypeInfoCaseClass(1)) should equal( """{"class":"za.co.absa.shaded.jackson.module.scala.ser.JsonTypeInfoCaseClass","intValue":1}""")
  }

  it should "serialize a case class containing a case class with JsonTypeInfo" in {
    serialize(CaseClassContainingJsonTypeInfoCaseClass(JsonTypeInfoCaseClass(1))) should equal( """{"c":{"class":"za.co.absa.shaded.jackson.module.scala.ser.JsonTypeInfoCaseClass","intValue":1}}""")
  }
